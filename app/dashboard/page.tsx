    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <section className="grid gap-3 grid-cols-2 sm:gap-4 lg:grid-cols-4">
        {statConfig.map(({ key, label, icon: Icon, color, bg, ring, accent }) => (
          <div
            key={key}
            className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-card sm:rounded-2xl sm:p-5"
          >
            ...
            <div className="flex items-start justify-between gap-2 sm:gap-3">
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400 truncate sm:text-[11px]">{label}</p>
                <p className="mt-1.5 text-2xl font-bold text-slate-800 sm:mt-2.5 sm:text-3xl">{statValues[key]}</p>
              </div>
              <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ring-1 sm:h-10 sm:w-10 sm:rounded-xl", bg, ring, color)}>
                <Icon />
              </div>
            </div>
          </div>
              ...
              {tasks.map((task) => {
                ...
                return (
                  <Link key={task.id} href={`/projects/${task.project_id}`} className="block group">
                    <div className={cn(
                      "flex flex-col gap-2 rounded-xl border bg-white px-4 py-3 shadow-card transition-all duration-150 sm:flex-row sm:items-center sm:gap-3",
            ...
            <div className="space-y-2">
              {recentProjects.map((project) => (
                <Link key={project.id} href={`/projects/${project.id}`} className="block group">
                  <div className="flex flex-col gap-1 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-card transition-all duration-150 sm:flex-row sm:items-center sm:justify-between sm:gap-3 group-hover:shadow-card-hover group-hover:-translate-y-0.5 group-hover:border-teal-200">
                    <div className="min-w-0 flex-1">
